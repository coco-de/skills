...
# Placeholder ZIP 생성 (deploy/aws/ 하위)
cd deploy/aws/
mkdir -p lambda/python/{pdf_router,pdf_processor,pdf_page_processor,pdf_aggregator,pdf_prepare_batches,book_batch_processor}/build
mkdir -p lambda/python/layers/pymupdf/build/x86_64
mkdir -p lambda/dart/sqs_to_sns/build/x86_64
mkdir -p lambda/go/fcm_topic_manager/build/x86_64

# 각 디렉토리에 placeholder ZIP 생성
for dir in lambda/python/*/build lambda/python/layers/pymupdf/build/x86_64 lambda/dart/sqs_to_sns/build/x86_64 lambda/go/fcm_topic_manager/build/x86_64; do
  echo "placeholder" | zip > "$dir/$(basename $dir).zip" 2>/dev/null
done
```
...
> **주의**: 이 프로젝트는 workspace로 state를 분리하지만, `config.auto.tfvars`에서 `enable_*_server` 플래그로 모든 환경을 단일 state에서 관리합니다.
