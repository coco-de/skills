---
name: coui-carousel
description: Activate when creating image carousels, slideshows, content sliders with autoplay, indicators, or arrow navigation using CouiCarousel/CouiCarouselItem (Flutter) or Carousel/CarouselItem (Web) in CoUI Flutter or CoUI Web.
---
...
CouiCarousel(
  items: [
    CouiCarouselItem(child: Image.network('url1')),
    CouiCarouselItem(child: Image.network('url2')),
  ],
  showIndicator: true,
  showArrows: true,
)
...
|-----------|------|---------|-------------|
| `items` | `List<CouiCarouselItem>` | required | Slides to display |
...
| Aspect | Flutter | Web |
|--------|---------|-----|
| Main widget | `CouiCarousel` | `Carousel` |
| Item widget | `CouiCarouselItem` | `CarouselItem` |
